...
export default function Settings() {
  ...
  const clearPredefinedPasswordInputRef = useRef<HTMLInputElement>(null)
  ...
  useEffect(() => {
    if (!showClearPredefinedModal) return
    const t0 = setTimeout(() => clearPredefinedPasswordInputRef.current?.focus(), 0)
    const t1 = setTimeout(() => clearPredefinedPasswordInputRef.current?.focus(), 100)
    const t2 = setTimeout(() => clearPredefinedPasswordInputRef.current?.focus(), 200)
  ...
  return (
    <div>
      ...
      {showClearPredefinedModal && (() => {
        const lng = i18n.language || 'pl'
        const T = (key: string, opts?: Record<string, unknown>) => t(key, { ...opts, lng })
        return (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 9999 }} onClick={() => !clearPredefinedLoading && setShowClearPredefinedModal(false)}>
          <div className="card" style={{ minWidth: 360 }} onClick={e => e.stopPropagation()} key={lng}>
            ...
            <div className="form-group">
              ...
              <input
                ref={clearPredefinedPasswordInputRef}
                type="password"
            ...
            <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end', marginTop: 16 }}>
              ...
              <button
                ...
                onClick={async () => {
                  ...
                  if (!pwd) return
                  const locale = lng.toLowerCase().slice(0, 2)
                  ...
                  try {
                    if (predefinedModalMode === 'restore') {
                      ...
                      if (!result?.ok) {
                        ...
                        setTimeout(() => clearPredefinedPasswordInputRef.current?.focus(), 150)
                        return
                      }
                      ...
                      if (res?.ok) {
                        ...
                        setTimeout(() => { window.api?.app?.refocusWindow?.() }, 0)
                        load()
