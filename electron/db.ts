...
export async function initDb() {
  ...
  // Nie wstawiamy tutaj domyślnej kategorii ani jednostek – przy pustej bazie preset (ensurePredefinedSettings) załaduje pełne dane z pliku. Fallback jest w ensurePredefinedSettings.
...
/** Weryfikuje hasło bieżącego użytkownika (do potwierdzenia krytycznych akcji). Gdy brak sesji – fallback na jedynego użytkownika w bazie. */
export function verifyCurrentUserPassword(password: string): boolean {
  ...
  const count = get<{ c: number }>('SELECT COUNT(*) AS c FROM app_users')?.c ?? 0
  if (count === 1) {
    const single = get<{ password_hash: string }>('SELECT password_hash FROM app_users LIMIT 1')
    if (single?.password_hash && verifyPassword(password, single.password_hash)) return true
  }
  return false
}

/** Wgrywa preset tylko przy całkowicie pustej bazie (brak kategorii, usług i zleceń). Zmiana języka przy kolejnym logowaniu nie nadpisuje danych – unikamy uszkodzenia istniejących zleceń. Gdy brak pliku presetu – wstawia minimalne domyślne (1 kategoria, words/pages/characters_with_spaces). */
export function ensurePredefinedSettings(uiLocale: string): void {
  ...
  // Fallback gdy brak pliku presetu – minimalne domyślne, żeby aplikacja działała
