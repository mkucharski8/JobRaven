...
/** Weryfikacja hasła tak samo jak przy zmianie hasła: przez serwer gdy jest sesja, inaczej lokalna baza. */
async function verifyPasswordForCriticalAction(password: string): Promise<boolean> {
  const meta = readAuthMeta()
  const email = meta.session?.email
  if (email) {
    const result = await changePasswordOnServer(email, password ?? '', password ?? '')
    return result.ok
  }
  return verifyCurrentUserPassword(password ?? '')
}
...
function registerIpcHandlers() {
  const reloadDbForCurrentOrganization = async () => {
    ...
    try {
      await initDb()
      const locale = (dbApi.settings.get('ui_language') || 'pl').toLowerCase().slice(0, 2)
      ensurePredefinedSettings(locale)
    } catch (err) {
      const msg = err instanceof Error ? err.message : String(err)
      if (msg === 'DB_ORGANIZATION_MISMATCH') {
        writeAuthMeta({ session: null, organizations: [] })
  ...
  ipcMain.handle('settings:clearPredefinedPreset', async (_, password: string) => {
    const ok = await verifyPasswordForCriticalAction(password ?? '')
